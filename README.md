...
| `/游戏名搜索 [游戏名]` | OMNI Search 游戏名称搜索，查询在线人数、访问量、好评率，附更多匹配候选 |
| `/游戏ID搜索 [数字ID]` | 查询游戏详情（兼容 Universe ID / 地点 ID，地点 ID 自动转换） |
...
- 菜单图片补充账号绑定命令（/绑定 /绑定ID /解绑 /查询）
- 绑定验证失败时区分「账号资料读取不到」与「简介未检测到验证码」两种提示
- 菜单图片行高改用中文字符测量；清理游戏渲染路径中不可达的旧平台分支

**新增功能：**
- 游戏名搜索显示更多匹配候选（前 3 个，含在线人数与 Universe ID），可换目标继续用 /游戏ID搜索 查询
- /游戏ID搜索 通过 placeId→universeId 转换接口恢复地点ID查询（games?placeIds 参数上游已 400）
...
- **第三方代理接口可能随时变化**（2026-08-29 更新）：
  - 游戏名搜索：原 `games/list` 接口已被 Roblox 上游下线（404），v1.6.0 起改用 OMNI Search，功能正常；
  - 游戏详情：`games?placeIds` 参数上游返回 400，`/游戏ID搜索` 已改用 placeId→universeId 转换接口支持地点ID输入；
  - 公开服务器列表（`servers/Public`）已被 Roblox 上游禁用（403），该功能已移除；
