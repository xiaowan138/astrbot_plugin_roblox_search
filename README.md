...
- **用户**：资料、头像与形象图、在线状态、注册时长、好友/粉丝/关注数、官方徽章；官机使用原生 Markdown 资料卡，OneBot 使用图文输出
...
**平台兼容**：同时兼容 **QQ 官方机器人（官机）** 与 **OneBot**（NapCat / Lagrange / LLOneBot 等）以及 Telegram、Discord 等其他 AstrBot 支持的平台。默认 `auto` 按每条消息来源自动适配，多平台同时接入也无需切换配置：官机的用户查询发送真正的 QQ 原生 Markdown 资料卡（不附带图片）；OneBot 用户查询保留头像框 + 虚拟形象图 + 纯文本。群组和游戏查询在官机上仍使用单图媒体消息，这是 QQ/AstrBot 当前不支持图片与原生 Markdown 同条发送的限制。
...
- **`platform_type`**（平台类型，默认 `auto`）
  ...
  - `qq_official`：手动指定 QQ 官方机器人。用户查询发送原生 Markdown 资料卡（不附带图片），并省略「正在查询...」中间提示（官方对消息条数/频率有限制），只回最终结果；群组和游戏查询仍使用单图媒体消息。
