...
**平台兼容**：同时兼容 **QQ 官方机器人（官机）** 与 **OneBot**（NapCat / Lagrange / LLOneBot 等）以及 Telegram、Discord 等其他 AstrBot 支持的平台。默认 `auto` 按每条消息来源自动适配，多平台同时接入也无需切换配置：官机用户查询依次发送一张 3D 虚拟形象图与一条原生 Markdown 资料卡；官机游戏查询发送原生 Markdown。OneBot 用户查询保留头像框 + 虚拟形象图 + 纯文本，游戏查询优先发送单张 HTML 游戏信息卡（渲染失败自动回退图标+文本）。图片与原生 Markdown 不能出现在同一条官机消息中，因此官机的形象图和资料卡会分两条发送。
...
- **`platform_type`**（平台类型，默认 `auto`）
  ...
  - `qq_official`：手动指定 QQ 官方机器人。用户查询依次发送 3D 虚拟形象图片与原生 Markdown 资料卡，游戏查询发送原生 Markdown；并省略「正在查询...」中间提示（官方对消息条数/频率有限制）。
